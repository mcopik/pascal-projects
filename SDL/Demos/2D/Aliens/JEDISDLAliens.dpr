...
begin
  result := DIR_CUR + 'data' + DIR_SEP + filename;
end;
...
{$ENDIF}
  ...
  // Load graphics
  player.image := LoadImage( PChar( DataFile('player.gif') ), True);
  ...
  shots[0].image := LoadImage( PChar( DataFile( 'shot.gif' ) ), False);
  ...
  aliens[0].image := LoadImage( PChar( DataFile( 'alien.gif' ) ), True);
  ...
  explosions[0].image := LoadImage( PChar( DataFile('explosion.gif') ), True);
  for i := 1 to MAX_ALIENS do
  begin
    explosions[i].image := explosions[0].image;
  end;

  background := LoadImage( PChar( DataFile( 'background.gif' ) ), False);
