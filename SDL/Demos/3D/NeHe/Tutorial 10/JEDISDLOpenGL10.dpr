...
type
  ...
  TVertex = record // vertex coordinates - 3d and texture
    x, y, z: GLfloat; // 3d coords.
    u, v: GLfloat; // texture coords.
  end;
...
var
  ...
  walkbias: GLfloat = 0;
  walkbiasangle: GLfloat = 0;
  lookupdown: GLfloat = 0.0;
  ...
  filter: GLuint = 0; // Which Filter To Use (nearest/linear/mipmapped)
  texture: array[0..2] of GLuint; // Storage For 3 Texture
...
begin
  ...
  TextureImage := SDL_LoadBMP('images/mud.bmp');
    ...
    try
      ...
      begin
        ...
        begin
          ...
          endPos := Pos( ' ', oneline );
          sector1.triangle[triloop].vertex[verloop].y := StrToFloat( Copy( oneline, 0, endPos - 1 ) );

          oneline := Copy(oneline, endPos + 1, Length(oneline) );
          endPos := Pos( ' ', oneline );
          sector1.triangle[triloop].vertex[verloop].z := StrToFloat( Copy( oneline, 0, endPos - 1 ) );

          oneline := Copy(oneline, endPos + 1, Length(oneline) );
          endPos := Pos( ' ', oneline );
          sector1.triangle[triloop].vertex[verloop].u := StrToFloat( Copy( oneline, 0, endPos - 1) );
