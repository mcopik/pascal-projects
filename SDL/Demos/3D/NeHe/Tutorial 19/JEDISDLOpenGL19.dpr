program JEDISDLOpenGL19;
{******************************************************************}
{                                                                  }
{       Borland Delphi SDL with OpenGL Example                     }
{       Conversion of the SDL OpenGL Examples                      }
{                                                                  }
{ Portions created by Ariel Jacob <ariel@global-rd.com>,  are      }
{ Copyright (C) 2002 Ariel Jacob.                                  }
{ All Rights Reserved.                                             }
{                                                                  }
{ The original files are : lesson19.c                              }
{                                                                  }
{ The original Pascal code is : JEDISDLOpenGL19.dpr                }
...
{ Revision History                                                 }
{ ----------------                                                 }
{   April   10 2002 - DL : Initial translation.                    }
{                                                                  }
...
var
  ...
  slowdown : single = 2.0; // Slow Down Particles
...
begin
  Status := FALSE;
  TextureImage := SDL_LoadBMP( 'images/Particle.bmp' );
...
begin
  ...
  // Random Fade Speed *)
  particles[ num ].fade := ( random( 10 ) mod 100 ) / 1000.0 + 0.003;
  ...
  case keysym.sym of
    ...
    SDLK_TAB :
      ...
      begin
        ...
        xi := ( ( random( 5 ) mod 50 ) - 26 ) * 10.0;
        yi := ( ( random( 5 ) mod 50 ) - 25 ) * 10.0;
        zi := yi;
...
begin
  ...
  begin
    color := Round( ( loop + 1 ) / ( MAX_PARTICLES / 12 ) );
    xi := ( ( random( 5 ) mod 50 ) - 26.0 ) * 10.0;
    yi := ( ( random( 5 ) mod 50 ) - 25.0 ) * 10.0;
    zi := yi;
    ResetParticle( loop, color, xi, yi, zi );
  end;
  Result := True;
...
begin
  ...
  begin
    ...
    begin
      ...
      particles[ loop ].life := particles[ loop ].life - particles[ loop ].fade;
      // If the particle dies, revive it */
      if ( particles[ loop ].life < 0.0 ) then
      begin
        xi := xspeed +
          random( 60 ) mod 60 - 32.0;
        yi := yspeed +
          random( 60 ) mod 60 - 30.0;
        zi := random( 60 ) mod 60 - 30.0;
...
begin
  ...
  SDL_WM_SetCaption( 'Jeff Molofee''s OpenGL Code Tutorial 19 using JEDI-SDL', nil );
  // get a SDL surface */
