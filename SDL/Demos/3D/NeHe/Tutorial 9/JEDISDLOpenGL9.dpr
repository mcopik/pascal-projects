...
var
  ...
  zoom: GLfloat = -15.0; // viewing distance from stars.
  tilt: GLfloat = 90.0; // tilt the view
  //spin: TGLfloat; // spin twinkling stars
  loop: GLuint; // general loop variable
  texture: GLuint; // Storage For 1 Texture
...
begin
  ...
  TextureImage := SDL_LoadBMP('images/star.bmp');
...
begin
  ...
  begin
    ...
    stars[loop].r := GLubyte( random(256) ); // random red intensity;
    stars[loop].g := GLubyte( random(256) ); // random green intensity;
    stars[loop].b := GLubyte( random(256) ); // random blue intensity;
  end;
...
begin
  ...
  begin
    ...
    // Assign A Color Using Bytes
    glColor4ub(stars[loop].r, stars[loop].g, stars[loop].b, GLubyte(255));
    glBegin(GL_QUADS); // Begin Drawing The Textured Quad
    glTexCoord2f(0.0, 0.0);
    glVertex3f(-1.0, -1.0, 0.0);
    glTexCoord2f(1.0, 0.0);
    ...
    begin
      ...
      stars[loop].r := GLubyte( random(256) ); // new red color.
      stars[loop].g := GLubyte( random(256) ); // new green color.
      stars[loop].b := GLubyte( random(256) ); // new blue color.
    end;
  end;
