...
{                                                                  }
{ The original Pascal code is : JEDISDLNewton1.dpr                 }
...
begin
  ...
  TextureImage := SDL_LoadBMP('images/newton_logo.bmp');
...
begin
  ...
  collision := NewtonCreateBox( nWorld, 2.0, 2.0, 2.0, nil );
  ...
  NewtonBodySetMassMatrix( ribidBodyBox, 5.0, 1.0, 1.0, 1.0 );

  // set the trasformation matrix
  Matrix_SetIdentity( matrix );
  Matrix_SetTransform( matrix, V3( 0, 0, 0 ) );
  NewtonBodySetMatrix( ribidBodyBox, @matrix[ 0, 0 ] );

  // animate the body by setting the angular veocity
  omega := V3( 15.0, 5.0, 5.0 );
  NewtonBodySetOmega( ribidBodyBox, @omega );
end;
...
begin
  ...
  SDL_WM_SetCaption( 'Newton Tutotial 1: Getting Started', nil );
