unit odeimport;
{
  $Id: odeimport.pas,v 1.13 2007/05/20 20:27:45 savage Exp $
}
{*************************************************************************
 *                                                                       *
...
{
  $Log: odeimport.pas,v $
  Revision 1.13  2007/05/20 20:27:45  savage
  Initial Changes to Handle 64 Bits

  Revision 1.12  2004/08/14 22:54:30  savage
  Updated so that Library name defines are correctly defined for MacOS X.

  Revision 1.11  2004/08/01 22:25:22  savage
  Commented out the EXT_* functions and changed
  dCreateSphere to dCreateTriMesh to access the DLL functions directly.
  Thanks for Jason Bell for this suggestion, which now allows the TruckOff ODE demo to work on with Delphi, Kylix and FreePascal under Win32 and Linux.
  Other changes are : Chroma dded support for the new QuickStep solver.

  Revision 1.10  2004/06/03 16:17:11  savage
  CH - New single and double dll. Added support for the new QuickStep solver.

  Revision 1.9  2004/05/10 14:10:03  savage
  Initial MacOS X support. Fixed defines for MACOS ( Classic ) and DARWIN ( MacOS X ).

  Revision 1.8  2004/04/25 16:36:52  savage
  CH - Trimesh now works in both mode single and double
  MF - Fixes to make DelphiODE behave better when used dynamically
  CH - New single and double dll. Now handles Capped Cylinder vs Trimesh collision Added dJointGetUniversalAngle and dJointGetUniversalAngleRate ...

  Revision 1.7  2004/04/07 22:06:55  savage
  Cross compiler changes/corrections for successfull compilation under FreePascal and Delphi.

  Revision 1.6  2004/04/05 09:38:09  savage
  Now handles trimesh/trimesh collision thanks to Christophe

  Revision 1.5  2004/03/30 22:47:28  savage
  uses module loader for better DLL/Shared object loading across platforms.

  Revision 1.4  2004/03/11 18:19:14  savage
  Thanks for Christophe
  Better trimesh/box collision code
  New placeable dterrain geom
  Built-in auto-disable functions
  breakable joints

  Breakable joints are not fully working (crash when asked to delete
  joint and not working with stepfast). Christophe contacted the author on the mailing list and he said he will look into it.

  Christophe has the Auto-disable feature disabled by default
  ...
  dCreateTerrainY
  dCreateTerrainZ
  dGeomTerrainYPointDepth
  dGeomTerrainZPointDepth

  dJointSetBreakable
  dJointSetBreakCallback
  dJointSetBreakMode
  dJointSetBreakForce
  dJointSetBreakTorque
  dJointIsBreakable
  dJointGetBreakForce
  dJointGetBreakTorque

  dJointCorrectHinge2

  Revision 1.3  2004/03/11 16:27:30  savage
  *** empty log message ***

  Revision 1.2  2004/03/10 11:17:29  savage
  Added breakable joints functions and added better Linux IFDEFS as suggested by Mattias and Christophe.
...
function dGeomCCylinderPointDepth( const Geom : PdxGeom; const x, y, z : TdReal ) : TdReal; cdecl; external{$IFDEF __GPC__}name 'dGeomCCylinderPointDepth'{$ELSE}ODEDLL{$ENDIF __GPC__};
  ...
  // ***************
function dCreateSphere( const Space : PdxSpace; const radius : TdReal ) : PdxGeom; cdecl; external{$IFDEF __GPC__}name 'dCreateSphere'{$ELSE}ODEDLL{$ENDIF __GPC__};
function dCreateBox( const Space : PdxSpace; const lx, ly, lz : TdReal ) : PdxGeom; cdecl; external{$IFDEF __GPC__}name 'dCreateBox'{$ELSE}ODEDLL{$ENDIF __GPC__};
function dCreatePlane( const Space : PdxSpace; const a, b, c, d : TdReal ) : PdxGeom; cdecl; external{$IFDEF __GPC__}name 'dCreatePlane'{$ELSE}ODEDLL{$ENDIF __GPC__};
function dCreateCCylinder( const Space : PdxSpace; const radius, length : TdReal ) : PdxGeom; cdecl; external{$IFDEF __GPC__}name 'dCreateCCylinder'{$ELSE}ODEDLL{$ENDIF __GPC__};
function dCreateCylinder( const Space : PdxSpace; r, lz : TdReal ) : PdxGeom; cdecl; external{$IFDEF __GPC__}name 'dCreateCylinder'{$ELSE}ODEDLL{$ENDIF __GPC__};
function dCreateCone( const Space : PdxSpace; radius, length : TdReal ) : PdxGeom; cdecl; external{$IFDEF __GPC__}name 'dCreateCone'{$ELSE}ODEDLL{$ENDIF __GPC__};
function dCreateTerrainY( const Space : PdxSpace; pHeights : PdRealHugeArray; vLength : TdReal; nNumNodesPerSide : Integer; bFinite, bPlaceable : Integer ) : PdxGeom; cdecl; external{$IFDEF __GPC__}name 'dCreateTerrainY'{$ELSE}ODEDLL{$ENDIF __GPC__};
function dCreateTerrainZ( const Space : PdxSpace; pHeights : PdRealHugeArray; vLength : TdReal; nNumNodesPerSide : Integer; bFinite, bPlaceable : Integer ) : PdxGeom; cdecl; external{$IFDEF __GPC__}name 'dCreateTerrainZ'{$ELSE}ODEDLL{$ENDIF __GPC__};
function dCreateRay( const Space : PdxSpace; length : TdReal ) : PdxGeom; cdecl; external{$IFDEF __GPC__}name 'dCreateRay'{$ELSE}ODEDLL{$ENDIF __GPC__};
function dCreateGeomTransform( const Space : PdxSpace ) : PdxGeom; cdecl; external{$IFDEF __GPC__}name 'dCreateGeomTransform'{$ELSE}ODEDLL{$ENDIF __GPC__};
function dCreateTriMesh( const Space : PdxSpace; Data : PdxTriMeshData; Callback, ArrayCallback, RayCallback : Pointer ) : PdxGeom; cdecl; external{$IFDEF __GPC__}name 'dCreateTriMesh'{$ELSE}ODEDLL{$ENDIF __GPC__};
