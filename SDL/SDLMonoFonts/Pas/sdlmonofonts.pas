...
begin
  ...
  // Search first/next separator
    while SDL_GetPixel( Image, x, 0 ) <> Separator do
      inc( x );
...
procedure ReadWord( Txt : PChar; StartPos : cardinal; var FoundWord : PChar; var ItsLength : cardinal; var EndOfLineReached : boolean );
var
  WasLetter    : boolean;
  ...
  repeat
    case Txt[ ReadPos ] of
      ...
      begin
        ...
        if (ReadPos+1 < TextLength) and (Txt[ReadPos+1] = #10) then
          inc( ReadPos, 1 );
        break;
      end;
...
begin
  if ( Surface = nil ) or ( Image = nil ) or ( Txt = nil ) or ( Txt = '' ) then
    exit;
  Original_Clip_Rect := Surface.Clip_Rect;
  if Rect = nil then
    Area := Surface.Clip_Rect
  else
    Area := Rect^;
  ...
  // A row always contains minimum one word
    ReadWord(Txt, ReadFrom, FoundRow, WordLen, EndOfLineReached); // Read a whole word from text
  ...
  // Read more words if it fits in Area's width
    repeat
      if EndOfLineReached then
        break; // We found end-of-line so break the line
      ReadWord(Txt, ReadFrom, FoundWord, WordLen, EndOfLineReached); // Read a whole word from text
      NextWordsLengthInPixels := WidthOf( FoundWord, WordLen );
      if RowLengthInPixels + Rects[ 0 ].w + 1 + NextWordsLengthInPixels < Area.w then
      begin
...
begin
  ...
  while i < len do
  begin
    if x >= Surface.Clip_Rect.x + Surface.Clip_Rect.w then
      break; // We reached the right side
    ch := pos( Txt[ i ], Charset ) - 1;
    if ( ch >= 0 ) and ( ch < 113 ) then
    begin
