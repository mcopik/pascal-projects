...
{
    Copyright (C) 2001  R€bert Kisn»meth
...
var
  ...
  FullScreen : UInt32 = SDL_FULLSCREEN;
  Next_Time : UInt32 = 0;
  GameMap : TMap;
...
destructor TFakePlayer.Destroy;
begin
  if Glass <> nil then
    SDL_FreeSurface( Glass );
  inherited;
...
destructor TSwimmingLetter.Destroy;
begin
  Image := nil;
  inherited;
end;
...
{$ENDIF}
  ...
  // Try to set the best videomode
  Screen := SDL_SetVideoMode( 640, 480, 0, SDL_SWSURFACE or SDL_HWPALETTE );
  if Screen = nil then
  begin
