...
{******************************************************************************}
{
  $Id: sdlwebhttp.pas,v 1.4 2005/04/24 19:06:45 drellis Exp $
...
{ ----------------                                                             }                                                                          
{
  ...
  Revision 1.4  2005/04/24 19:06:45  drellis
  Updated HTTP POST to handle the HTTP/1.1 100 Continue message.

  Revision 1.3  2005/04/17 16:56:53  drellis
  Updated HTTP POST to send Content-Type so that it will work is sites written with ASP.
...
type
  ...
  TResponseResult = ( rsOK, rsContinue, rsError, rsProxyAutheticate );
...
begin
  ...
  begin
    ...
    begin
      ...
      SDLWeb_SendRequest( Connection, 'Content-Type: application/x-www-form-urlencoded' );
    end;
...
begin
  ...
  repeat
    ...
    begin
      ...
      if status = 100 then
      begin
        if CompareText( copy( s, 1, 21 ), 'HTTP/'+HTTP_VER+' 100 Continue' ) = 0 then
        begin
          Result := rsContinue;
        end;
      end;
    ...
    if (S = EmptyStr) and (Result = rsContinue) then
    begin
