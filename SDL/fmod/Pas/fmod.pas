...
unit fmod;
{
  $Id: fmod.pas,v 1.5 2007/05/20 20:21:40 savage Exp $
  
}
(*
  $Log: fmod.pas,v $
  Revision 1.5  2007/05/20 20:21:40  savage
  Initial Changes to Handle 64 Bits

  Revision 1.4  2004/10/20 23:12:32  savage
  Updated to 3.74

  Revision 1.3  2004/08/14 22:54:30  savage
  Updated so that Library name defines are correctly defined for MacOS X.

  Revision 1.2  2004/05/10 14:10:04  savage
  Initial MacOS X support. Fixed defines for MACOS ( Classic ) and DARWIN ( MacOS X ).

  Revision 1.1  2004/03/30 21:52:34  savage
  Moved to it's own folder.

  Revision 1.7  2004/02/25 23:38:29  savage
  Fix to FSOUND_Stream_GetTagField, making sure that the last parameter is a var Lengh : integer.
  Thanks to Steve Williams for point this out.

  Revision 1.6  2004/02/25 11:39:51  savage
  Added compiler directive to maintain FPC v1.0 compatability.

  Revision 1.5  2004/02/25 10:39:20  savage
  Added oldfpccall; to
  procedure set8087cw( value : word ); oldfpccall; assembler; so that it compiles under v1.9.2 of FPC compiler.

  Revision 1.4  2004/02/20 17:28:40  savage
  Previous comments stopped code from compiling. Now fixed

  Revision 1.3  2004/02/19 15:00:38  savage
  Changed {$IFDEF to WIN32 instead of MSWINDOWS}

  Revision 1.2  2004/02/14 00:56:55  savage
  Added UNIX support for FreeBSD

  Revision 1.3  2004/02/05 00:37:23  savage
...
function FSOUND_GetDriverName(Id: Integer): PChar; {$IFDEF UNIX} cdecl {$ELSE} stdcall {$ENDIF};
...
function FSOUND_Sample_Load(Index: Integer; const NameOrData: PChar; Mode: Cardinal; Offset: Integer; Length: Integer): PFSoundSample; {$IFDEF UNIX} cdecl {$ELSE} stdcall {$ENDIF};
...
function FSOUND_Sample_GetName(Sptr: PFSoundSample): PCHAR; {$IFDEF UNIX} cdecl {$ELSE} stdcall {$ENDIF};
...
function FSOUND_Stream_Open(const name_or_data: PChar; Mode: Cardinal; Offset: Integer; Length: Integer): PFSoundStream; {$IFDEF UNIX} cdecl {$ELSE} stdcall {$ENDIF};
...
function FSOUND_Stream_AddSyncPoint(Stream: PFSoundStream; PCMOffset: Cardinal; Name: PChar): PFSyncPoint; {$IFDEF UNIX} cdecl {$ELSE} stdcall {$ENDIF};
...
function FSOUND_Stream_GetSyncPointInfo(Point: PFSyncPoint; var PCMOffset: Cardinal): PCHAR; {$IFDEF UNIX} cdecl {$ELSE} stdcall {$ENDIF};
...
function FSOUND_Stream_GetTagField(Stream: PFSoundStream; Num: Integer; var TagType: TFSoundTagFieldType; var Name: PChar; var Value: Pointer; var Length: Integer): ByteBool; {$IFDEF UNIX} cdecl {$ELSE} stdcall {$ENDIF};
function FSOUND_Stream_FindTagField(Stream: PFSoundStream; TagType: TFSoundTagFieldType; Name: PChar; var Value: Pointer; var Length: Integer): ByteBool; {$IFDEF UNIX} cdecl {$ELSE} stdcall {$ENDIF};

{
    Internet streaming functions
}

function FSOUND_Stream_Net_SetProxy(Proxy: PChar): ByteBool; {$IFDEF UNIX} cdecl {$ELSE} stdcall {$ENDIF};
function FSOUND_Stream_Net_GetLastServerStatus(): PChar; {$IFDEF UNIX} cdecl {$ELSE} stdcall {$ENDIF};
...
function FSOUND_Record_GetDriverName(Id: Integer): PChar; {$IFDEF UNIX} cdecl {$ELSE} stdcall {$ENDIF};
...
function FMUSIC_LoadSong(const Name: PChar): PFMusicModule; {$IFDEF UNIX} cdecl {$ELSE} stdcall {$ENDIF};
...
function FMUSIC_GetName(Module: PFMusicModule): PCHAR; {$IFDEF UNIX} cdecl {$ELSE} stdcall {$ENDIF};
